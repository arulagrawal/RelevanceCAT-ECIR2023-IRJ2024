"""
Baseline evaluation on TREC DL'19 — cross-encoder WITHOUT score injection.
Used as the control to compare against SPLADECAT.
"""
import os
...
model_name = "finetuned_CEs/train-cross-encoder-kd-baseline-microsoft-MiniLM-L12-H384-uncased/"  # TODO: update with actual path after training
fine_tuned_model_path = model_name
ranking_output_path = model_name + "trec19_baseline.ranking"
...
model_max_length = 230 + 3  # [cls]query[sep]doc[sep] — NO extra tokens for injection
...
class CrossEncoder():
    ...
    def predict(self, sentences: List[List[str]], batch_size: int = 32, show_progress_bar: bool = None,
               num_workers: int = 0, activation_fct = None, apply_softmax = False,
    ...
    def rank(self, model) -> float:
        ...
        try:
            ...
            for instance in tqdm.tqdm(self.samples):
                ...
                # No injection — same query for all docs
...
def read_top1000_run(f_path, corpus, queries, allowed_qids=None):
    """Read top-1000 run file — NO score injection, plain queries."""
    samples = {}
    with open(f_path, "r") as fp:
        for line in tqdm.tqdm(fp, desc="reading {}".format(f_path)):
...
test_samples = read_top1000_run(top100_run_path, corpus, queries, allowed_qids=allowed_qids)
