"""
BM25CAT evaluation on TREC DL'19 cross-encoder WITH BM25 score injection.

Used as the reproduction model to compare against:
- baseline: no score injection
- SPLADECAT: SPLADE score injection
...
## TODO: update with the actual path after training
model_name = "train/finetuned_CEs/train-cross-encoder-kd-bm25cat-microsoft-MiniLM-L12-H384-uncased-2026-04-07_13-20-48-latest/"
fine_tuned_model_path = model_name
ranking_output_path = model_name + "trec19_bm25cat.ranking"

base_write_path = ""
base_path = "train/msmarco-data/"
...
## BM25 scores for TREC DL 2019 top-1000 candidates
scores_path = injection_scores_path + "3_trec19_bm25_scores.json"

## BM25 normalization constants - must match training script!
global_min_bm25 = 0
global_max_bm25 = 50
...
print(
    "fine_tuned_model_path {} | model_max_length {} | queries_path {} | ranking_output_path {}".format(
        fine_tuned_model_path, model_max_length, queries_path, ranking_output_path
    )
)

if not os.path.exists(scores_path):
    raise FileNotFoundError(
        "Missing BM25 TREC19 score file: {}\n"
        "You need to create this file for the same TREC19 top1000 candidate set.".format(scores_path)
    )

scores = json.loads(open(scores_path, "r", encoding="utf8").read())
...
class CrossEncoder():
    def __init__(
        ...
        if activation_fn is not None:
            ...
            try:
                self.config.sbert_ce_default_activation_function = util.fullname(self.activation_fn)
            except Exception as e:
                logger.warning("Was not able to update config about the activation_fn: {}".format(str(e)))
            ...
            self.activation_fn = util.import_from_string(self.config.sbert_ce_default_activation_function)()
        else:
            self.activation_fn = nn.Sigmoid() if self.config.num_labels == 1 else nn.Identity()
    ...
    def predict(
        ...
        with torch.no_grad():
            for features in iterator:
                ...
                logits = activation_fn(model_predictions.logits)
...
class CERerankingEvaluatorTest:
    ...
    def rank(self, model) -> float:
        ...
        try:
            ...
            if self.qrel is not None and self.qrel != "None":
                ...
                scores_eval = evaluator.evaluate(run)
                self.mean_metrics = {}
                metrics_string = ""

                for metric in list(self.all_metrics):
                    self.mean_metrics[metric] = np.mean([ele[metric.replace(".", "_")] for ele in scores_eval.values()])
    ...
    def __fix_rank_filed(self, ranking_path="", splittor=" ", return_lines=False, ranking_lines=None, remove_last_query=None):
        ...
        list_of_tupple = []
        for query, docs_dict in re_rank_dict.items():
...
def get_truncated_dict(id_content_dict, tokenizer, max_length):
    for id_, content in tqdm.tqdm(id_content_dict.items()):
        truncated_content = tokenizer.batch_decode(
            tokenizer(
                content,
                padding=True,
                truncation=True,
...
with open(qrel_path, 'r', encoding="utf8") as f_qrel:
...
allowed_qids = set({
    '19335', '47923', '87181', '87452', '104861', '130510', '131843', '146187',
    '148538', '156493', '168216', '182539', '183378', '207786', '264014', '359349',
    '405717', '443396', '451602', '489204', '490595', '527433', '573724', '833860',
    '855410', '915593', '962179', '1037798', '1063750', '1103812', '1106007', '1110199',
    '1112341', '1113437', '1114646', '1114819', '1115776', '1117099', '1121402',
    '1121709', '1124210', '1129237', '1133167'
})

test_samples = read_top1000_run(
    top100_run_path,
    corpus,
    queries,
    separator=" ",
